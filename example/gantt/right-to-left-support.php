...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($type == "dependency") {
        $columns = ['ID', 'PredecessorID', 'SuccessorID', 'Type'];

        $table = "GanttDependencies";
    } else {
        $columns = ['ID', 'ParentID', 'OrderID', 'Title', 'Start', 'End', 'PercentComplete', 'Expanded', 'Summary'];
...
$create->url('right-to-left-support.php?type=task&operation=create')
     ->contentType('application/json')
     ->type('POST');

$read = new \Kendo\Data\DataSourceTransportRead();

$read->url('right-to-left-support.php?type=task&operation=read')
     ->contentType('application/json')
     ->type('POST');

$update = new \Kendo\Data\DataSourceTransportUpdate();

$update->url('right-to-left-support.php?type=task&operation=update')
     ->contentType('application/json')
     ->type('POST');

$destroy = new \Kendo\Data\DataSourceTransportDestroy();

$destroy->url('right-to-left-support.php?type=task&operation=destroy')
     ->contentType('application/json')
     ->type('POST');
...
$create->url('right-to-left-support.php?type=dependency&operation=create')
     ->contentType('application/json')
     ->type('POST');

$read = new \Kendo\Data\DataSourceTransportRead();

$read->url('right-to-left-support.php?type=dependency&operation=read')
     ->contentType('application/json')
     ->type('POST');

$update = new \Kendo\Data\DataSourceTransportUpdate();

$update->url('right-to-left-support.php?type=dependency&operation=update')
     ->contentType('application/json')
     ->type('POST');

$destroy = new \Kendo\Data\DataSourceTransportDestroy();

$destroy->url('right-to-left-support.php?type=dependency&operation=destroy')
     ->contentType('application/json')
     ->type('POST');
