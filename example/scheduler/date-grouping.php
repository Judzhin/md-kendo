...
?>
 <div class="box wide">
    <div class="box-col">
        ...
        <?php
        $size = new \Kendo\UI\DropDownList('orientation');
...
<script>
    function onChange() {
        ...
        scheduler.options.group.orientation = dropdownlist.value();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $columns = ['MeetingID', 'Title', 'Start', 'End', 'StartTimezone', 'EndTimezone', 'IsAllDay', 'Description', 'RecurrenceID', 'RecurrenceRule', 'RecurrenceException', 'RoomID'];

    switch ($type) {
        case 'create':
            $result = $result->createWithAssociation('Meetings', 'MeetingAttendees', $columns, $request->models, 'MeetingID', ['Attendees' => 'AttendeeID']);
            break;
        case 'update':
            $result = $result->updateWithAssociation('Meetings', 'MeetingAttendees', $columns, $request->models, 'MeetingID', ['Attendees' => 'AttendeeID']);
            break;
        case 'destroy':
            $result = $result->destroyWithAssociation('Meetings', 'MeetingAttendees', $request->models, 'MeetingID');
            break;
        default:
            $result = $result->readWithAssociation('Meetings', 'MeetingAttendees', 'MeetingID', ['AttendeeID' => 'Attendees'], ['MeetingID', 'RoomID'], $request);
            break;
    }
...
$create->url('resources-grouping-horizontal.php?type=create')
     ->contentType('application/json')
     ->type('POST');

$read = new \Kendo\Data\DataSourceTransportRead();

$read->url('resources-grouping-horizontal.php?type=read')
     ->contentType('application/json')
     ->type('POST');

$update = new \Kendo\Data\DataSourceTransportUpdate();

$update->url('resources-grouping-horizontal.php?type=update')
     ->contentType('application/json')
     ->type('POST');

$destroy = new \Kendo\Data\DataSourceTransportDestroy();

$destroy->url('resources-grouping-horizontal.php?type=destroy')
     ->contentType('application/json')
     ->type('POST');
...
$scheduler->timezone("Etc/UTC")
        ...
        ->group(['date' => true, 'resources' => ['Rooms']])
        ->addView(['type' => 'day', 'startTime' => new DateTime('2013/6/13 7:00')],
            ['type' => 'week', 'selected' => true, 'startTime' => new DateTime('2013/6/13 7:00')], 'month', 'timeline');
...
<script>
    $(document).ready(function () {      
        var scheduler = $("#scheduler").data("kendoScheduler");
        var dropdownlist = $("#orientation").data("kendoDropDownList");
        dropdownlist.value(scheduler.options.group.orientation);                    
    });
</script>
