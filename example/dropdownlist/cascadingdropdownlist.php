...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    switch ($type) {
        case 'categories':
            $result = $result->read('Categories', ['CategoryID', 'CategoryName'], $request);
            break;
        case 'products':
            $result = $result->read('Products', ['ProductID', 'ProductName', 'CategoryID'], $request);
            break;
        case 'orders':
            $result = $result->read('[Order Details]', ['OrderID', 'ProductID', 'Quantity'], $request);
            break;
    }
...
$categories->dataSource(['transport' => $transport, 'schema' => $schema, 'serverFiltering' => true])
...
$products->dataSource(['transport' => $transport, 'schema' => $schema, 'serverFiltering' => true])
         ->autoBind(false)
