...
class DataSourceResult
{
    ...
    {
        $sort = isset($request->sort) && count($request->sort) ? $request->sort : [];
        $groups = isset($request->group) && count($request->group) ? $request->group : [];
    ...
    private function groupBy($data, $groups, $table, $request, $propertyNames)
    {
        if (count($groups) > 0) {
            ...
            $result = [];
            $value = $data[0][$field];
            $aggregates = isset($groups[0]->aggregates) ? $groups[0]->aggregates : [];
